...
function getSheetData() {
  try {
    ...
    var idxTidakDitemukan = headers.indexOf("TIDAK DITEMUKAN");
    var idxNikKosong = headers.indexOf("NIK KOSONG");
    if (idxNikKosong === -1) idxNikKosong = headers.indexOf("NIK ANGGOTA KELUARGA KOSONG");
    ...
    if (idxTidakDitemukan === -1) idxTidakDitemukan = 19; // Col T
    if (idxNikKosong === -1) idxNikKosong = 20; // Col U
    if (idxProgres === -1) idxProgres = 21;     // Col V
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      rows.push({
        ...
        tidakDitemukan: Number(r[idxTidakDitemukan]) || 0,
        nikKosong: Number(r[idxNikKosong]) || 0,
    ...
    var lastUpdateDate = "";
    try {
      var tanggalSheet = ss.getSheetByName("tanggal") || ss.getSheetByName("Tanggal");
      if (tanggalSheet) {
        lastUpdateDate = tanggalSheet.getRange("A1").getDisplayValue().toString().trim();
      }
    } catch (e) {
      // ignore
    }
    
    return { success: true, data: rows, lastUpdateDate: lastUpdateDate };
  } catch (e) {
